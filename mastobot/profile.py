...
def get_id_info(id: str) -> dict:
    ...
    """
    url_id = f'https://mastodon.social/api/v1/accounts/{id}'
...
def get_following(id: str, limit: str) -> dict:
    ...
    """
    url = f'https://mastodon.social/api/v1/accounts/{id}/following?limit={limit}'
    response = http_client.rq_get(
        url= url, headers= login
    )
    return json.loads(response.text)
...
def get_followers(id: str, limit: str) -> dict:
    ...
    """
    url = f'https://mastodon.social/api/v1/accounts/{id}/followers?limit={limit}'
    response = http_client.rq_get(
        url= url, headers= login
    )
    return json.loads(response.text)